...
interface CandidateCompareDockProps {
  selectedCandidates: SelectedCandidateItem[];
...
export const CandidateCompareDock: React.FC<CandidateCompareDockProps> = ({
  selectedCandidates,
  onRemove,
  onClear,
  onCompare,
    ...
    <AnimatePresence>
      {count > 0 && (
        <motion.div
          ...
          className="fixed bottom-6 inset-x-0 z-40 max-w-4xl mx-auto px-4 pointer-events-none"
        >
          <div className="pointer-events-auto bg-white/95 dark:bg-slate-900/95 backdrop-blur-xl border-2 border-indigo-500/30 dark:border-indigo-400/30 rounded-2xl p-3.5 shadow-2xl shadow-indigo-500/10 dark:shadow-indigo-950/40 flex flex-col sm:flex-row items-center justify-between gap-3">
            {/* Left: Info & Chips */}
              ...
              <div className="flex items-center gap-1.5 overflow-x-auto no-scrollbar">
                {selectedCandidates.map((cand, idx) => (
                  <motion.div
                    ...
                    className="flex items-center gap-1.5 bg-slate-100 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 px-2.5 py-1 rounded-xl text-xs text-slate-800 dark:text-slate-200 font-medium shrink-0 max-w-[150px]"
                  >
            ...
            <div className="flex items-center gap-2 w-full sm:w-auto justify-end shrink-0">
              ...
              <motion.button
                ...
                className={`px-4 py-2 rounded-xl text-xs font-semibold flex items-center gap-1.5 shadow-md transition-all ${
                  isReady
                    ? "bg-gradient-to-r from-indigo-600 to-purple-600 hover:from-indigo-500 hover:to-purple-500 text-white shadow-indigo-500/25 cursor-pointer ring-2 ring-indigo-400/40"
