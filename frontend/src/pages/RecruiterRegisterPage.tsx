    ...
    return (
      <AnimatedPage className="min-h-screen flex items-center justify-center px-4">
        <div className="bg-white dark:bg-slate-800 rounded-2xl border p-10 text-center max-w-sm">
          ...
          <h2 className="font-display text-xl font-bold mb-2">{t.alreadyRecruiter}</h2>
          <Link to="/dashboard" className="block w-full py-2.5 bg-indigo-600 text-white rounded-xl text-sm">{t.toDashboard}</Link>
        </div>
      </AnimatedPage>
    );
  }
  ...
  return (
    <AnimatedPage className="min-h-screen bg-slate-50 dark:bg-slate-900">
      <div className="max-w-lg mx-auto px-4 py-10">
        <h1 className="font-display text-3xl font-bold mb-2">{t.recruiterRegTitle}</h1>
        <p className="text-sm text-slate-500 mb-6">{t.recruiterRegDesc}</p>
        {latest && !pending && (
          <div className="mb-6 p-4 rounded-xl bg-slate-100 dark:bg-slate-800 border text-sm">
            ...
            {t.latestApplication(latest.status, formatDate(latest.created_at, false, lang))}
            {latest.admin_note && <p className="mt-1 text-xs">{t.adminNote}: {latest.admin_note}</p>}
          </div>
        )}
        <form onSubmit={handleSubmit} className="bg-white dark:bg-slate-800 rounded-2xl border border-slate-200 dark:border-slate-700 p-6 space-y-3">
          <div className="relative">
            ...
            <input required value={form.companyName} onChange={(e) => setForm((p) => ({ ...p, companyName: e.target.value }))} placeholder={t.companyNamePlaceholder} className="w-full pl-10 pr-3 py-2.5 bg-slate-50 dark:bg-slate-700/50 border border-slate-200 dark:border-slate-600 rounded-xl text-sm" />
          </div>
          <input value={form.companyEmail} onChange={(e) => setForm((p) => ({ ...p, companyEmail: e.target.value }))} placeholder={t.companyEmailPlaceholder} className="w-full px-3 py-2.5 bg-slate-50 dark:bg-slate-700/50 border border-slate-200 dark:border-slate-600 rounded-xl text-sm" />
          <input value={form.website} onChange={(e) => setForm((p) => ({ ...p, website: e.target.value }))} placeholder={t.websitePlaceholder} className="w-full px-3 py-2.5 bg-slate-50 dark:bg-slate-700/50 border border-slate-200 dark:border-slate-600 rounded-xl text-sm" />
          <input value={form.licenseUrl} onChange={(e) => setForm((p) => ({ ...p, licenseUrl: e.target.value }))} placeholder={t.licenseUrlPlaceholder} className="w-full px-3 py-2.5 bg-slate-50 dark:bg-slate-700/50 border border-slate-200 dark:border-slate-600 rounded-xl text-sm" />
          ...
          <Button
            ...
            fullWidth
          >
            {pending ? t.updateForm : t.submitForm}
          </Button>
        </form>
      </div>
    </AnimatedPage>
