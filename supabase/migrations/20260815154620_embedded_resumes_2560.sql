-- qwen3.7-text-embedding max dim is 2560. Existing 4096-d vectors cannot be recast.
...
alter table public.embedded_resumes
  add column embedding extensions.vector(2560) not null;
...
drop function if exists public.match_resumes_for_job(extensions.vector(4096), uuid, int);

create or replace function public.match_resumes_for_job (
  query_embedding extensions.vector(2560),
  p_job_id uuid,
